     ...
     const [loading,setLoading] = useState(false)
        ...
        const handleSendOtp = async () => {
            ...
            if (emailPattern.test(email)) {
                try {
                    setLoading(true)
                  const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/operator/send-otp`,{email},{
        ...
        const handleVerifyOtp = async()=>{
            try {
                setLoading(true)
                const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/operator/verify-otp`,{Otp},{
                    withCredentials:true
                })
                ...
                if(response.data){
                    ...
                    dispatch(SetUser(response?.data?.data?.operator))
                    //  close dialog & reset
                    setOpen(false)
                    setStep(1)
                    setOtp("")
                    Setemail("")
                    // close sidebar
                    navigate("/operator-dashboard/dashboard") 
                }
            } catch (error) {
  ...
  return (
        <Dialog open={Open} onOpenChange={(isOpen)=>{
            ...
            <DialogTrigger asChild>
                <button onClick={()=>setOpen(true)} className='bg-white text-[#D63941] font-semibold px-6 py-3 rounded-full hover:bg-gray-100 transition'>Register now</button>
            </DialogTrigger>
           
            <DialogContent overlayClassName="bg-black/20" className="sm:max-w-[550px] w-full mx-auto">
...
export default OperatorLoginDialog